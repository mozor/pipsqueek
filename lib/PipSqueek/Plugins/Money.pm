package PipSqueek::Plugins::Money;
...
sub plugin_initialize
{
	...
	$self->plugin_handlers(
		'multi_money' => 'currency_conversion',
	);
}
...
sub currency_conversion
{
	...
	if( $message->message() =~ m/money\s+codesearch\s+(.+)$/ )
	{
my $CODES = {
...
"Zimbabwe" => ["ZWD", "Dollar"],
		...
		my @matches = grep(/\Q$input\E/i, keys %$CODES);

		if( @matches ) {
			@matches = map { "$_ ($CODES->{$_}->[0])" } @matches;
			local $"=', ';
			return $self->respond( $message, "Found matches: @matches" );
		}
		else
		{
	...
	if ( $amount eq '_' )
	{
		$amount = $Heap->{'last_math_result'} || 1;
	}
	...
	if( $from eq $to )
	{
		...
		"Convert between two _different_ currencies nimwit..." );
	}
	...
	{
		return $self->respond( $message, 
			"HTTP Request Failed: " . $response->status_line() );
	}
